from PyFun.commons import saveMax
...
smalldata = [["3"], ["7", "4"],["2", "4", "6"],["8", "5", "9", "3"]]

def step(maximums, newValues):
    maxima = [0] + maximums + [0]
    newMaxima = []
    for i in range(len(newValues)):
        newMaxima.append(
            calculateMax(int(newValues[i]), maxima[i], maxima[i+1])
        )
    return newMaxima


def calculateMax(current, poss1, poss2):
    return current  + saveMax([poss1, poss2])

def solve(d):
    maximums = []
    for i in range(len(d)):
        maximums = step(maximums, d[i])
        print maximums
    return saveMax(maximums)

print solve(data).getOrElse("Error")